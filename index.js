...
var MODE_NAMES = state.MODE_NAMES;
state = state.state;
...
if (checkManualMode()) {
  ...
  state.mode = MODES.AWAY;
  state.target_temp = 4;
}

var system_poweroff = function() {
  relays._signal_all(1);
  state.mode = MODES.POWEROFF;
  refresh();
  lcd.off();
...
button(15, function(delta) {
  if (checkManualMode()) {
    ...
    //console.log('mode');
    if (state.mode > MODES._LAST) return;
    if (state.mode == MODES._LAST) state.mode = MODES._FIRST;
    else ++state.mode;
    state.target_temp = {1:22, 2:20, 3:4}[state.mode];
    refresh();
  }
})
...
refresh = function() {
    ...
    lcdPromise = lcdPromise.then(function() {
      ...
      if (state.mode <= MODES._LAST) {
        var s = "Nacin: "+MODE_NAMES[state.mode]+" ";
        if (state.mode == MODES.AWAY) {
          s += "         ";
        } else {
          s += state.target_temp.toFixed(1)+String.fromCharCode(0xdf)+"C   ";
        }
        return lcd.print(s);
      } else {
        return lcd.print("   "+MODE_NAMES[state.mode]+"      ");
      }
    }).then(function() {
...
var L1TempEstimator = (function() {
  ...
  return {
    ...
    get: function() {
      ...
      var fix = (floor-24)*0.24; // adjust per floor temp (neutral at 26 deg)
      return temp + fix;
    },
  }
})();
...
function decide() {
  ...
  // L e v e l - 0
  if (overheat) {
    relays.L0_pump.set(sensors.L0_ret.temp < 60 || sensors.L0_pump.temp < 60);
  } else {
    relays.L0_pump.off();
  }
  
  // L e v e l - 1
  if (state.mode<=MODES._LAST && (supply > target+2 || sensors.stove.temp > 55) || overheat) {
...
var process_loop = 0;
function process() {
  if ((state.mode > MODES._LAST) && !checkManualMode()) {
    state.mode = MODES.DAY;
    state.target_temp = 22;
    refresh();
  } else if ((state.mode <= MODES._LAST) && checkManualMode()) {
    state.mode = MODES.MANUAL;
    refresh();
  }
